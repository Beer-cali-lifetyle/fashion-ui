(function($) {
    ...
    setBottomOverLayerPosition(2e3);
    setupSwiper();
            ...
            $(document.body).on("appear", ".pie-chart-style-01", function(e) {
                $(".pie-chart-style-01").easyPieChart({
                    ...
                    barColor: function() {
                        ...
                        gradient.addColorStop(.2, color1);
    ...
    const curvedTextAnimation = (target, options) => {
        let duration = options.duration ? options.duration <= 2e3 ? 2e3 : options.duration : 2e3,
    ...
    const waveTextAnimation = (target, options) => {
        let duration = options.duration ? options.duration : 3e3,
    ...
    function FancyTextDefault(item, ftOptions) {
        let text_effect = ftOptions.effect,
            duration = ftOptions.duration ? ftOptions.duration : 3e3,
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (redirectVal != "" && redirectVal != undefined) {
                        ...
                        resultsObj.removeClass("d-none").fadeIn("slow").delay(4e3).fadeOut("slow")
                    }
                }
            })
        }
        ...
        if (!isIE()) {
            if (typeof $.fn.parallax !== "undefined" && typeof $.fn.parallax !== null) {
                $("[data-parallax-background-ratio]").each(function() {
                    var ratio = $(this).attr("data-parallax-background-ratio") || .5;
    ...
    if ($(".magic-cursor").length > 0) {
        ...
        if (typeof TweenMax !== "undefined" && typeof TweenMax !== null) {
            $(".magic-cursor").mouseenter(function(e) {
                TweenMax.to("#ball-cursor", .3, {
                    borderWidth: "2px",
                    scale: 1
                });
                TweenMax.to("#ball-cursor-loader", .2, {
                    borderWidth: "2px",
                    top: 2,
                    left: 2
                });
    ...
    function onScroll() {
        if (window.sections.length > 0) {
            ...
            }).find(section => section.rect.bottom >= window.innerHeight * .5);
